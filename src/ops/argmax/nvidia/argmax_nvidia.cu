...
// ValIdx for shared memory reduction (always float for comparison)
struct ValIdx {
    float val;
    int idx;
};

// Argmax kernel: single block reduction, compare in float
template<typename T>
...
void argmax(tensor_t max_idx, tensor_t max_val, tensor_t vals) {
    ...
    case LLAISYS_DTYPE_BF16:
        argmax_kernel<__nv_bfloat16><<<1, threads, smem_size>>>(
            (int *)max_idx->data(), (__nv_bfloat16 *)max_val->data(),
            (const __nv_bfloat16 *)vals->data(), numel, stride);
        break;
    default:
        throw std::runtime_error("NVIDIA argmax: unsupported dtype");
    }
